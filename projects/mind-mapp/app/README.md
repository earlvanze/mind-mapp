...
- Fit to view (F) + Fit selection (Alt+F) + Fit focused subtree (Alt+Shift+F) + Center focused node (C) + Center selection (Alt+Shift+C) + Center focused subtree (Alt+Shift+B) + Center root (Shift+C) + Focus parent/child/siblings (Shift+P/N/H/J) + Focus root (R) + Focus backtrack (Alt+R) + Grid overlay toggle (Shift+G) + Reset View button
