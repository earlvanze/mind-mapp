...
test('recognize title applies handwriting to the node title', async ({ page }) => {
  await page.addInitScript(() => {
    window.mindMappRecognizeHandwriting = async strokes => {
      if (!strokes.length || !strokes[0].length) throw new Error('no strokes')
      return 'handwritten title'
    }
    localStorage.setItem('mind-mapp-v1', JSON.stringify({
      nodes: [{ id: 1, x: 120, y: 120, text: 'Topic', width: 80, height: 40 }],
      edges: [],
  ...
  await page.locator('#btn-recognize-title').click()
  await expect(page.locator('#details-node-title')).toHaveValue('handwritten title')
  await expect(page.locator('#recognition-status')).toContainText('Recognized title')

  const saved = await page.evaluate(() => JSON.parse(localStorage.getItem('mind-mapp-v1')))
  expect(saved.nodes[0].text).toBe('handwritten title')
})
