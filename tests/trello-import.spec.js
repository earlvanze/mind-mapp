...
test('imports the real Trello mindmap JSON as consolidated project groups', async ({ page }) => {
  ...
  expect(rootEdges.every(edge => ['east', 'west', 'north', 'south'].includes(edge.side))).toBe(true)
  expect(rootEdges.some(edge => edge.side === 'north')).toBe(true)
  expect(rootEdges.some(edge => edge.side === 'south')).toBe(true)
